...
let playerSign = "1"; //this is a global variable beacuse we've used this variable inside multiple functions
...
function clickedBox(element){
    if(players.classList.contains("player")){
        playerSign = "2"; //if player choose (2) then change playerSign to O
...
function bot(){
    ...
    if(runBot){ //if runBot is true
        playerSign = "2"; //change the playerSign to 2 so if player has chosen 1 then bot will 2
        ...
        if(array.length > 0){ //if array length is greater than 0
            if(players.classList.contains("player")){ 
                playerSign = "1"; //if player has chosen O then bot will X
        ...
        playerSign = "1"; //if player has chosen X then bot will be 2 right then we change the playerSign again to 1 so user will 1 because above we have changed the playerSign to 2 for bot
    }
}
...
function selectWinner(){ //if the one of following winning combination match then select the winner
    if(checkIdSign(1,2,3,playerSign) || checkIdSign(4,5,6, playerSign) || checkIdSign(7,8,9, playerSign) || checkIdSign(1,4,7, playerSign) || checkIdSign(2,5,8, playerSign) || checkIdSign(3,6,9, playerSign) || checkIdSign(1,5,9, playerSign) || checkIdSign(3,5,7, playerSign)){
        ...
        wonText.innerHTML = `Jugador <p>${playerSign}</p> ganó el juego!`; //displaying winning text with passing playerSign (1 or 2)
